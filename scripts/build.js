...
async function optimizeImageAssets() {
  ...
  try {
    await imagemin([path.resolve(__dirname, '../assets/img/*.{jpg,png,svg}')], {
      destination: path.join(buildDir, 'assets/img'),
...
async function build() {
  ...
  copySync(
    path.resolve(__dirname, '../assets/video'),
    path.join(buildDir, 'assets/video')
  )
